...
(function () {
  ...
  DEFAULTS.forEach(function (s) {
    state.data[s] = { px: null, chg: 0, series: [] };
  });
  ...
  async function fetchQuote(sym) {
    try {
      var r = await fetch('/api/quote?symbol=' + encodeURIComponent(sym) + '&_=' + Date.now(), {
        cache: 'no-store'
      });
      var j = await r.json();
      var px = j?.px ?? null;
      ...
      if (px != null) {
        state.data[sym].px = px;
        state.data[sym].chg = chg;

        var s = state.data[sym].series;
        s.push(px);
        if (s.length > HISTORY_LEN) s.shift();
      }
    } catch (e) {
      // mantém último valor, sem travar
      // opcional: console.warn('quote error', e);
    }
  }

  // Atualização periódica
  async function periodic() {
    // Atualiza o ativo focado primeiro (UX)
    await fetchQuote(state.active);

    // Atualiza os da lista, sem estourar quota
    var others = Object.keys(state.data).filter(function (s) { return s !== state.active; });
    for (var i = 0; i < others.length; i++) {
      await fetchQuote(others[i]);
      // micro-pausa para evitar estouro agressivo
  ...
  $("alertBtn").onclick = function () {
    ...
    if (px != null) openAlert(s, "above", (px * 1.02).toFixed(isBR(s) ? 2 : 2));
  };
  ...
  // Atalhos: duplo clique abre o modal completo
